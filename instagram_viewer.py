    ...
    def _make_client(self, login: str, pwd: str) -> Client:
        ...
        if session_file.exists():
            try:
                ...
                # Проверяем сессию через user_info — НЕ account_info (тот делает edit=true → 403)
                # Берём pk из кук, делаем лёгкий запрос user_info_by_username на себя
                ds_user_id = cl.cookie_dict.get("ds_user_id", "")
                if ds_user_id:
                    # Самый лёгкий способ: просто обратиться к публичному профилю
                    # через уже установленные куки — не триггерит блокировку
                    cl.user_id = int(ds_user_id)
                    cl.username = login
                    # Делаем запрос к user_info по pk — не требует edit=true
                    user = cl.user_info(int(ds_user_id))
                    logger.info(f"[pool] ✅ сессия загружена: {login} (@{user.username}, id={user.pk})")
                else:
                    # Нет ds_user_id в куках — пробуем по юзернейму
                    user = cl.user_info_by_username(login)
                    logger.info(f"[pool] ✅ сессия загружена: {login} (id={user.pk})")
                return cl
            except Exception as e:
                err_str = str(e).lower()
                # Если сессия протухла — сообщаем и не пытаемся login()
                if any(x in err_str for x in ("login_required", "not authorized", "challenge")):
                    logger.error(
                        f"[pool] ❌ сессия {login} протухла: {e}\n"
                        f"👉 Обнови куки в браузере и пересоздай {login}.json"
                    )
                    raise RuntimeError(f"Сессия {login} протухла — нужно обновить куки") from e
                logger.warning(f"[pool] сессия {login}: проверка не удалась ({e}), продолжаю...")
                return cl  # Возвращаем клиент даже без проверки — пусть пробует работать

        # Файла сессии нет совсем
        raise RuntimeError(
