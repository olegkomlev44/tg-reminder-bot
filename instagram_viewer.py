...
def _session_dir() -> Path:
    # Приоритет: явная переменная → /app/data (BotHost) → /data → рядом со скриптом
    candidates = []
    if os.getenv("IG_SESSION_DIR"):
        candidates.append(os.getenv("IG_SESSION_DIR"))
    if os.getenv("DB_DIR"):
        candidates.append(os.path.join(os.getenv("DB_DIR"), "ig_sessions"))
    candidates += ["/app/data/ig_sessions", "/data/ig_sessions"]
    candidates.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "ig_sessions"))
    for c in candidates:
        p = Path(c)
        try:
    ...
    def _make_client(self, login: str, pwd: str) -> Client:
        ...
        if session_file.exists():
            try:
                ...
                # Проверяем сессию лёгким запросом — БЕЗ login() чтобы не светить IP
                info = cl.account_info()
                logger.info(f"[pool] ✅ сессия загружена без логина: {login} (id={info.pk})")
                return cl
            except Exception as e:
                logger.warning(f"[pool] сессия {login} невалидна ({e}), пробую login...")
                cl = Client()
                cl.delay_range = [2, 5]

        # Полный логин (только если нет файла сессии или сессия устарела)
        if not pwd or pwd in ("anypassword", "x", "unused"):
            raise RuntimeError(
                f"Сессия для {login} не найдена/устарела, а пароль не задан. "
                "Обнови куки через ig_session_from_cookies.py"
            )
        cl.login(login, pwd)
