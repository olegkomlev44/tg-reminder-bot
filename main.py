...
async def cmd_start(message: types.Message):
    ...
    "🎵 *Музыка* — `/find <название>` или `/charts`. "
    "Качаю mp3, делаю обложки, рекомендую похожее, "
    "плейлисты, очередь, DJ-голосование — полный комбайн.\n\n"
    "📸 *Instagram* — `/ig <username>` анонимно смотришь "
    "истории и посты любого открытого профиля. "
    "Подпишись — буду кидать новые автоматом.\n\n"
    "🧠 `/tldr` — токсичная выжимка вашего флуда.\n"
    "☁️ `/pogoda [город]` — погода + совет как одеться.\n"
    "🃏 `/meme` + фото — демотиватор за 3 секунды.\n"
    "🎨 `/poll <текст>` или `/imagen <текст>` — рисую картинки.\n\n"
