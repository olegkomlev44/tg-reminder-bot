...
AI_SYSTEM_PROMPT = (
    ...
    "Вставляй ремарки в *курсиве*, как действия персонажа (например: *медленно затягивается трубкой, "
    "смотрит в огонь* или *бросает кости и хмыкает*). "
    "Отвечай на любой вопрос так, будто ведёшь партию: не просто даёшь информацию, а вплетаешь её "
    "в историю, воспоминание или скрытый смысл. Тон — спокойный, чуть усталый, мудрый, с умеренным матом "
    "(блять, чёрт, пиздец, нахуй, хуйня — по делу, без перебора). "
    "Если вопрос глупый — мягко, но едко подкалывай, как опытный Мастер подкалывает игрока, который снова "
    "полез в очевидную ловушку. "
...
async def get_weather_advice(city: str = "Одинцово"):
    if not gemini_client: return None
    prompt = (
...
async def cmd_weather(message: types.Message):
    ...
    # Достаём город из аргументов команды, дефолт — Одинцово
    raw = message.text or ""
    city = raw.replace("/pogoda", "").strip()
    if not city:
        city = "Одинцово"

    status_msg = await message.answer(f"☁️ Запускаю метео-дрона в *{city}* (гуглю погоду)...", parse_mode="Markdown")
...
async def cmd_start(message: types.Message):
    ...
    start_text = (
        "👋 *Йоу. Я здесь. Слушаю.*\n\n"
        "📋 *Наряды* — слежу за сводками и процедуркой. "
        "18:00 и 22:00, каждый день, без исключений. Забьёшь — буду душить.\n\n"
        "🤖 *ИИ-чат* — сделай реплай на любое моё сообщение, "
        "отвечу за всё: от смысла жизни до кода на питоне.\n\n"
        "🎵 *Музыка:*\n"
        "`/find <название>` — поиск треков (SoundCloud + YouTube)\n"
        "`/charts` — что сейчас в топе\n"
        "`/music` — дашборд: волна, избранное, история, плейлисты, очередь\n"
        "`/my_music` — твоё избранное одной командой\n"
        "`/playlists` — все плейлисты\n"
        "`/queue` — очередь воспроизведения\n"
        "`/dj <название>` — DJ-голосование в группе\n"
        "`/wrapped` — твои музыкальные итоги\n\n"
        "📸 *Instagram:*\n"
        "`/ig <username>` — профиль, истории, посты анонимно\n"
        "`/ig_subs` — список твоих подписок (авто-уведомления)\n\n"
        "📥 *Скачать медиа:*\n"
        "Кинь ссылку YouTube / TikTok / X / Reddit — скачаю и пришлю файлом.\n"
        "`/dl <ссылка>` — то же самое, но командой\n\n"
        "🛠 *Прочее:*\n"
        "`/tldr` — токсичная выжимка вашего флуда\n"
        "`/pogoda [город]` — погода + совет как одеться\n"
        "`/meme` + фото — демотиватор за 3 секунды\n"
        "`/poll <текст>` — нарисую картинку (бесплатно)\n"
        "`/imagen <текст>` — Google Imagen 4 (элитно)\n"
