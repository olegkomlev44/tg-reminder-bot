...
async def api_track_info(request):
    ...
    track = await music_engine.get_track_details(tid)
    if not track: return cors(web.json_response({"error": "not found"}, status=404))
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(track["stream_url"], headers=hdrs) as up:
                ...
                rh = {"Content-Type": ct, "Accept-Ranges": "bytes", "Cache-Control": "no-cache", "Access-Control-Allow-Origin": "*"}
